...
- 界面文案明确说明搁置变更和 Git 暂存栈的恢复入口差异，避免用户误解为自动恢复。
- 当搁置与降级暂存都无法继续时，默认跳过当前仓库并继续其他安全仓库；只有用户明确取消时才终止剩余处理。
...
- 日志、通知、错误提示均使用中文。
- 仓库选择、刷新、切换、状态图标和结果状态需补充可访问名称或说明，且状态表达不能只依赖颜色。
